...
Python conventions for this repo. Adapted from [Development principles § Code conventions](../../development-principles.md) and aligned with **linux-whisper** where applicable.
...
- Secrets and machine-specific deploy paths via `.env` (gitignored; copy from `.env.example`) — never committed
