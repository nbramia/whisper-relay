"""HTTP client for the Hermes assistant (LifeOS-compatible SSE subset)."""
...
class HTTPHermesBackendClient:
    """Hermes at HERMES_BACKEND_URL — no LifeOS handoff path (ADR-004)."""
        ...
        async with httpx.AsyncClient(timeout=self._timeout, headers=self._headers()) as client:  # noqa: SIM117
            async with client.stream(
                ...
                if resp.status_code != 200:
                    ...
                    raise LifeOSError(
                        f"hermes backend returned HTTP {resp.status_code}: {raw[:500]!r}"
                    )
    ...
    async def list_conversations(self, *, persona_id: str | None = None) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=30.0, headers=self._headers()) as client:
            ...
            if resp.status_code != 200:
                raise LifeOSError(f"hermes backend returned HTTP {resp.status_code}")
            return resp.json()
